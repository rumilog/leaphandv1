...
public class HandController : MonoBehaviour
{
    ...
    // Simple quaternion total-angle difference (shortest arc), in degrees
    static float AngleDiffDeg(Quaternion a, Quaternion b)
    {
        float d = Mathf.Abs(Quaternion.Dot(a, b));
        d = Mathf.Clamp(d, -1f, 1f);
        return 2f * Mathf.Acos(d) * Mathf.Rad2Deg;
    }
